...
static char *BF_crypt(__CONST char *key, __CONST char *setting, char *output, int size,
					  BF_word min) {
	struct {
		BF_ctx ctx;
		BF_key expanded_key;
	...
	if (setting[0] != '$' || setting[1] != '2' || (setting[2] != 'a' && setting[2] != 'x') ||
		setting[3] != '$' || setting[4] < '0' || setting[4] > '3' || setting[5] < '0' ||
		setting[5] > '9' || (setting[4] == '3' && setting[5] > '1') || setting[6] != '$') {
		__set_errno(EINVAL);
		return NULL;
	}
...
char *_crypt_blowfish_rn(__CONST char *key, __CONST char *setting, char *output, int size) {
#ifdef BF_SELF_TEST
	__CONST char *test_key = "8b \xd0\xc1\xd2\xcf\xcc\xd8";
	__CONST char *test_2a = "$2a$00$abcdefghijklmnopqrstuui1D709vfamulimlGcq0qq3UvuUasvEa"
							"\0"
							"canary";
	__CONST char *test_2x = "$2x$00$abcdefghijklmnopqrstuuVUrPmXD6q/nVSSp7pNDhCR9071IfIRe"
							"\0"
							"canary";
...
char *_crypt_gensalt_blowfish_rn(unsigned long count, __CONST char *input, int size, char *output,
								 int output_size) {
